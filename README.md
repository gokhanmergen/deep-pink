...
- **Temporary chats**, for the question you do not want a record of. There is no separate way to start one: a switch in the title bar — or `Ctrl/⌘ ⌥ T` — turns the chat you are already in into one. It appears only on a chat nothing has been done to yet — unnamed, unpinned, unfiled, nothing said in it — because deciding a conversation should leave no trace is a decision to take before having it rather than after. That is also what makes the silent deletion safe: those are exactly the threads the startup sweep would remove anyway, so no other machine can be left holding a copy of one. From then on it lives exactly as long as you are in it: open another chat and it is deleted, close the app and it is deleted, and if the app never got the chance — a crash, a pulled plug — it is deleted on the next start instead. Everything it took with it goes too: the messages, the pictures, what each turn cost. It is never named by the naming model, because paying for a label on something about to stop existing is not a good trade, and it is **never synced** — not the conversation, not the fact that there was one. Deletions normally leave a tombstone behind so they can travel to your other machines; a temporary chat's are swept up in the same transaction, so a bucket never learns that on this day this machine had a conversation of this length. Changed your mind? The same switch turns it back — press `Ctrl/⌘ ⌥ T` again, click it again, or use the context menu — and the chat becomes an ordinary thread with its id and costs intact. That direction has no conditions on it: keeping a conversation is allowed right up until the moment it would have gone. Pinning it, filing it in a folder or archiving it does the same thing, because all three of those mean "later".
- A long conversation opens at the speed of a short one. The transcript is read in from the end and the rest arrives as you scroll back, a page at a time — asked for while it is still two screens away, so what you reach has already been there for a while. Your place is held to the pixel across each arrival: messages are inserted above the viewport and the view is put back before the browser paints, so nothing you are reading ever moves. What the thread cost is asked of the database rather than added up from what happens to be on screen, so the figure in the header is the whole thread's and does not creep upwards as you scroll. Syntax highlighting runs on a worker thread, so a reply full of code cannot stutter the window it is arriving in — and the browser is told it may skip laying out the messages it is not showing, which is what keeps the wheel tracking in a thread of hundreds.
...
- **Replies of several documents**, for an answer that is genuinely a set rather than one long thing — one write-up per file reviewed, one per region, one per option being compared. The model writes a fenced `dp-docs` block listing them and the app gives you a list to pick from; opening one replaces the list, and the way back sits in the header of what you are reading rather than above it, so it is still there once you have scrolled in. Arrows step between documents without going back to the list. Off by default, switchable per thread from the composer's **Extras** menu or with `Ctrl/⌘ ⇧ D`, and the exact paragraph the model is given is on screen in Settings › Documents. Each document is ordinary Markdown through the same renderer as the rest of a reply, so there is nothing a model can do here it could not do in a paragraph. **Deliberately not a filesystem**: no folders, no paths, no nesting, no links between documents — everything that would make it a tree is something you would then have to navigate. The list fills in as the reply is written — a document that has finished arriving is a complete document, so it appears rather than waiting behind a wall of JSON, and the header says it is still coming. A reply that was cut off mid-set still shows the documents that did arrive.
- Web access, charts and documents share one **Extras** menu in the composer rather than a button each. They are the settings you change least — once a thread is set up you do not touch them again — and between them they were taking more of the composer than the box you type into. The button says how many are on, which is the part worth seeing at a glance.
...
- And choose **which way each of them travels**, because "sync my settings" is not one wish. Both ways is the default and does what it says. *Only send* makes this machine the one that decides — its settings go up, and nothing the others do comes back down. *Only receive* makes it the one that follows. The choice is per scope, so a laptop can take a desktop's prompts and shortcuts while its conversations still travel in both directions. Within a direction the newest change to a given record wins, as before. Two things stay on the machine they were set on whatever you choose: the window's zoom, which is a fact about a screen rather than a preference, and the bucket and key themselves.
...
- Pause it when you want to: for an hour, until tomorrow morning, or until you say otherwise, from Settings › Sync or with `Ctrl/⌘ ⇧ U`. A pause stops the automatic runs and nothing else — the bucket, the key and your choices stay exactly as they are, **Sync now** still works, and a pause with a time on it lifts by itself even if the machine was asleep through it. A run already going stops where it is rather than finishing first; what it managed is recorded, and the next run carries on from there.
