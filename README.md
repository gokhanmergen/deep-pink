...
- Unsent prompts stay with their threads for the current session. They appear in the sidebar as yellow **WIP Thread** rows, and the draft text and coloring clear when the app restarts.
...
- Charts in replies: a model can answer with a line, area, bar, column or scatter chart by writing a fenced `dp-chart` block of JSON, and the app draws it — in the same palette, on the same grid and with the same crosshair as the statistics panels, because the model chooses the numbers and Deep Pink chooses everything else. The prompt switch is off by default, can be changed per thread from the composer or with `Ctrl/⌘ ⇧ B`, and controls whether the model receives chart instructions. Valid chart fences are rendered even when that switch is off. The exact paragraph of system prompt the model is given is on screen in Settings › Skills. **It is deliberately not HTML**: nothing a model writes is ever parsed as markup, so a chart cannot run a script, load a remote image, restyle the window or escape the message it is in. Everything is validated and clamped before it reaches a component; anything that does not validate stays the code block it was and says why, and the numbers behind any chart are one click away.
