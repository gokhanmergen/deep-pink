...
- **Temporary chats**, for the question you do not want a record of. `Ctrl/⌘ ⌥ N` starts one, and it lives exactly as long as you are in it: open another chat and it is deleted, close the app and it is deleted, and if the app never got the chance — a crash, a pulled plug — it is deleted on the next start instead. Everything it took with it goes too: the messages, the pictures, what each turn cost. It is never named by the naming model, because paying for a label on something about to stop existing is not a good trade, and it is **never synced** — not the conversation, not the fact that there was one. Deletions normally leave a tombstone behind so they can travel to your other machines; a temporary chat's are swept up in the same transaction, so a bucket never learns that on this day this machine had a conversation of this length. Changed your mind? **Keep this chat** in the title bar, the context menu or `Ctrl/⌘ ⌥ K` turns it into an ordinary thread, id and costs intact — and pinning it, filing it in a folder or archiving it does the same thing, because all three of those mean "later".
