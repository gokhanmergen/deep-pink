/**
 * Barrel of everything under test. Bundled once by scripts/run-tests.mjs so the
 * test files can require a single CommonJS artifact from inside Electron.
 */
export { getDb } from '@/db/index'
export * as repo from '@/db/repo'
export { streamChat, OpenRouterError } from '@/providers/openrouter'
export { shouldCompact, toChatParams } from '@/chat/engine'
export { assembleContext, estimateTokens, skillsFor } from '@/chat/prompt'
export * as skills from '@shared/skills'
export * as skillTool from '@/tools/skills'
export { htmlToText, runWebFetch, runWebSearch } from '@/tools/web'
export * as attachments from '@/attachments'
export * as chatgpt from '@/import/chatgpt'
export * as importer from '@/import/index'
export * as exporter from '@/export/index'
export * as archive from '@/export/archive'
export { toMarkdown } from '@/export/markdown'
export * as repoTools from '@/tools/repo'
export * as charts from '@shared/charts'
export * as docs from '@shared/docs'
export { DEFAULT_SETTINGS, MOST_KEY_POINTS, clampKeyPoints, keyPointCeiling } from '@shared/defaults'
export { takeKeyPointMarkers, keyPointPrompt } from '@shared/keyPointPrompt'
export { earliestOfTheBest } from '@/providers/typesafe'
export * as syncCrypto from '@/sync/crypto'
export * as s3 from '@/sync/s3'
export * as syncEngine from '@/sync/engine'
export * as syncRecords from '@/sync/records'
export * as secrets from '@/secrets'
